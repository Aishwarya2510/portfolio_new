 ...
 * recognize a returning visitor without authentication.
 */
...
export function optionalDb(ctx: APIContext): D1Database | null {
  return ctx.locals.runtime?.env?.DB ?? null;
}
...
export async function getCurrentVisitor(
  ...
  if (!id) return null;
  try {
    return await getVisitorById(ctx, id);
  } catch {
    return null;
  }
}
